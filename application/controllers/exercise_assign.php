...
class Exercise_assign extends CI_Controller{
    ...
    public function getGroups($gradeId,$classId)
    {
        $tid=1;
        $className="unknown";
        if ($classId==1){
        ...
        $result=$this->teachermanagement->get_group_by_teacher_class($tid,$gradeId,$className);
        echo json_encode($result);
    }

    public function getStudents($GID)
    {
    ...
    public function publish(){
        ...
        $ename = $kname."_".date("Y-m-d H:i:s",time());
        
        $exerciseIdSelected=$this->input->post('exerciseIdSelected');

        $enum=count($exerciseIdSelected);
        $econtent['ex']=$exerciseIdSelected;
        $econtent=json_encode($econtent);
    ...
    public function index(){
        ...
        $data['sitebar'] = 'exercise_assign';
        $this->cismarty->assign($data);
        $this->cismarty->display('exercise-assign.tpl');
    }
